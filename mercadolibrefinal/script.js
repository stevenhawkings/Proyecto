// Verificar si debemos mostrar storePage al cargar home.html
document.addEventListener('DOMContentLoaded', function() {
  const homePage = document.getElementById("homePage");
  const storePage = document.getElementById("storePage");
  
  if (homePage && storePage && sessionStorage.getItem('mostrarStorePage') === 'true') {
    homePage.style.display = "none";
    storePage.style.display = "block";
    mostrarProductos(productos);
    sessionStorage.removeItem('mostrarStorePage');
  }
});
...
if (offersBtn) {
  offersBtn.addEventListener("click", () => {
    sessionStorage.setItem('mostrarStorePage', 'true');
...
// Funcionalidad para abrir/cerrar carrito
const btnAbrirCarrito = document.getElementById("btnAbrirCarrito");
const btnCerrarCarrito = document.getElementById("btnCerrarCarrito");
const carritoAside = document.getElementById("carrito");

if (btnAbrirCarrito && carritoAside) {
  btnAbrirCarrito.addEventListener("click", () => {
    carritoAside.style.display = "block";
    carritoAside.style.right = "20px";
    carritoAside.style.top = "80px";
  });
}

if (btnCerrarCarrito && carritoAside) {
...
document.addEventListener("click", (e) => {
  if (carritoAside && 
      !carritoAside.contains(e.target) && 
      e.target !== btnAbrirCarrito &&
      !e.target.closest('.producto button') &&
      carritoAside.style.display === "block") {
    carritoAside.style.display = "none";
  }
});